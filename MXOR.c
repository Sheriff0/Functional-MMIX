...
unsigned long long yc = ((((~0llu/*invert all 64 zeroes to ones*/)<<63llu/*get rid of 63 ones to make "1000..."*/) >> (8llu*c)) >> (unsigned long long)j/*yc only obeys "j"*/);

 unsigned long long zc = ((((~0llu) << 63llu) >> (8llu*i)) >> (unsigned long long)c);

yc &= y;
zc &= z;
   /*if a 1 is copied after the ^ operation, shift right to make it the least significant bit (push to the right extreme)*/
  if(yc > 0ull){ 
  yc >>= ys;
  }
    if(yc > 0ull){ 
